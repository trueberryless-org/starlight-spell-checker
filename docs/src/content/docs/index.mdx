---
title: Starlight Spell Checker
description: Check your documentation for spelling mistakes; multilingual support.
head:
  - tag: title
    content: Starlight Spell Checker
template: splash
editUrl: false
hero:
  tagline: Check your documentation for spelling mistakes; multilingual support.
  image:
    dark: ../../assets/big-logo-dark.png
    light: ../../assets/big-logo-light.png
  actions:
    - text: Get Started
      link: /getting-started/
      icon: right-arrow
banner:
  content: |
    **Deprecation Notice:** This project will be **archived and unpublished from NPM on January 1, 2026** and will no longer receive updates or support. Please **do not use this package for new projects**.
---

import { Card, CardGrid } from '@astrojs/starlight/components'

## Next steps

<CardGrid stagger>
  <Card title="Install the plugin" icon="puzzle">
    Check the [getting started guide](/getting-started/) for installation instructions.
  </Card>
  <Card title="Configure the plugin" icon="setting">
    Edit your config in the `astro.config.mjs` file.
  </Card>
</CardGrid>

## Contributors

import { ContributorList } from "starlight-contributor-list";

<ContributorList githubRepo="trueberryless-org/starlight-spell-checker" ignore={["autofix-ci[bot]", "github-actions[bot]", "renovate[bot]"]} />
